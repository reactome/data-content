                                ...
                                <div>
                                    <c:forEach var="value" items="${crossReference.value}" varStatus="loop">
                                        <a href="${value.url}" target="_blank"
                                           title="show ${value.displayName}">${value.identifier}</a><c:if
                                            test="${!loop.last}">, </c:if>
                                    </c:forEach>
                                </div>
                            </div>
                        ...
                        <div class="favth-col-lg-3 favth-col-md-3 favth-col-sm-6 favth-col-xs-12 text-overflow">
                            <c:choose>
                                <c:when test="${not empty person.orcidId}">
                                    <a href="${detailRequestPrefix}person/${person.orcidId}" class=""
                                       title="${person.displayName}">${person.displayName} &nbsp;(<fmt:formatDate
                                            pattern="yyyy-MM-dd" value="${date}"/>)</a>
                                </c:when>
                                <c:otherwise>
                                    <a href="${detailRequestPrefix}person/${person.dbId}" class=""
                                       title="${person.displayName}">${person.displayName} &nbsp;(<fmt:formatDate
                                            pattern="yyyy-MM-dd" value="${date}"/>)</a>
                                </c:otherwise>
                            </c:choose>
